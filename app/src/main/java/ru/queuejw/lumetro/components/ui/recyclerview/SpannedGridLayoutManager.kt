package ru.queuejw.lumetro.components.ui.recyclerview
...
open class SpannedGridLayoutManager(
    ...
    protected open fun measureChild(position: Int, view: View): Rect {
        ...
        if (usedSpan > getSpanCountForOrientation() || usedSpan < 1) {
            throw InvalidSpanSizeException(
                errorSize = usedSpan,
                maxSpanSize = getSpanCountForOrientation()
            )
        }
    ...
    fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
        ...
        return PointF(
            if (orientation == HORIZONTAL) direction.toFloat() else 0f,
    ...
    protected open fun fillBefore(recycler: RecyclerView.Recycler) {
        ...
        val lastRow =
            (scroll + size - getPaddingStartForOrientation()) / getItemSizeForOrientation()
    ...
    override fun getDecoratedTop(child: View): Int {
        ...
        if (orientation == VERTICAL) top -= scroll
    ...
    override fun getDecoratedLeft(child: View): Int {
        ...
        if (orientation == HORIZONTAL) left -= scroll
    ...
    override fun getDecoratedBottom(child: View): Int {
        ...
        if (orientation == VERTICAL) bottom -= scroll - getPaddingStartForOrientation()
    ...
    protected open fun subtract(subtractedRect: Rect) {
        ...
        for (free in interestingRects) {
            if (free.isAdjacentTo(subtractedRect) && !subtractedRect.contains(free)) {
                ...
                if (free.left < subtractedRect.left) possibleNewRects.add(
                    Rect(
                        free.left,
                        free.top,
                        subtractedRect.left,
                ...
                if (free.right > subtractedRect.right) possibleNewRects.add(
                    Rect(
                ...
                if (free.top < subtractedRect.top) possibleNewRects.add(
                    Rect(
                        free.left,
                        free.top,
                        free.right,
                ...
                if (free.bottom > subtractedRect.bottom) possibleNewRects.add(
                    Rect(
                        free.left,
